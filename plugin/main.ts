...
export default class CloudObsidianPlugin extends Plugin {
	...
	async onload(): Promise<void> {
		...
		this.ribbonIconEl = this.addRibbonIcon("cloud-obsidian-sync", "Cloud Obsidian Sync", () => {
	...
	async manualPush(): Promise<void> {
		...
		for (const file of files) {
			...
			changes.push({ path: file.path, action: "update", content, clientMtime: file.stat.mtime });
		}
		if (changes.length > 0) {
	...
	private startSyncEngine(): void {
		...
		// File watcher detects local changes and triggers push.
		this.fileWatcher = new FileWatcher(
			this.app.vault,
			(changes: FileChange[]) => { if (this.syncEngine) this.syncEngine.push(changes); }
		);
		this.fileWatcher.start();

		// CRITICAL: wire FileWatcher into SyncEngine so pull ops
		// don't re-trigger the watcher (avoids sync loop → flickering).
	...
	private updateStatusBar(status: SyncStatus): void {
		const icons: Record<SyncStatus, string> = {
			online: "🟢", offline: "⚫", pushing: "🔼", pulling: "🔽", syncing: "🔄", error: "🔴",
		};
		const labels: Record<SyncStatus, string> = {
			online: "Synced", offline: "Offline", pushing: "Pushing...", pulling: "Pulling...", syncing: "Syncing...", error: "Error",
		};
